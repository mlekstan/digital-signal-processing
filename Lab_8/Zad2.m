...
fs = 400e3;                         % czestotliwosc próbkowania
fc = 100e3;                         % czestotliwosc nośna
M  = 1024;                          % połowa długosci filtra
N  = 2*M+1;
n  = 1:M;
...
xr1 = resample(x1, fs, fs1);        % resample(data,P,Q) - Ustawienie (Q/P) < 1 prowadzi do interpolacji, czyli zwiększenia częstotliwości próbkowania poprzez dodanie nowych próbek między istniejącymi próbkami.
Nxr1 = length(xr1);
...
t1 = length(x1)/fs1; % całkowity czas trwania x1
t  = 0:1/fs:t1-1/fs; % wektor próbek czasowych sygnału wyjsciowego 
