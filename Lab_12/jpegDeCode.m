function img = jpegCode( bits )

    bq = bits(1:16);
    bq_str = num2str(bq);
    bq_str = bq_str(~isspace(bin_str)); % Usuń spacje
    q = bin2dec(bq_str); % Zamień ciąg znaków binarnych na wartość dziesiętną
    
    bx = bits(17:32);
    bx_str = num2str(bx);
    bx_str = bx_str(~isspace(bin_str)); % Usuń spacje
    x = bin2dec(bq_str); % Zamień ciąg znaków binarnych na wartość dziesiętną

    by = bits(33:48);
    bx_str = num2str(bx);
    bx_str = bx_str(~isspace(bin_str)); % Usuń spacje
    y = bin2dec(bq_str); % Zamień ciąg znaków binarnych na wartość dziesiętną
