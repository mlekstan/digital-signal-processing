...
y1_error = abs(y1 - x.');
max_error_1 = max(abs(y1-x.'))                      % maks. błąd rekonstrukcji 
avg_error_1 = mean(abs(y1-x.'))                     % średni błąd rekonstrukcji

%% Z kwantyzacją
dq = lab11_kwant(d, Nb);                            % kwantyzator
y2 = dekoder(dq, a);                                % DEKODER
y2_error = abs(y2-x.');
max_error_2 = max(abs(y2-x.'))                      % maks. błąd rekonstrukcji
avg_error_2 = mean(abs(y2-x.'))                     % średni błąd rekonstrukcji
